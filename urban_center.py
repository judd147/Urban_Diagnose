...
def main():    
    ...
    with open('https://raw.githubusercontent.com/judd147/Urban_Diagnose/main/config.yaml') as file:
...
def reclassify(df):
    ...
    0.跳过一级分类为"事件活动|交通设施服务|公共设施|地名地址信息|室内设施|摩托车服务|汽车服务|汽车维修|汽车销售|通行设施|道路附属设施"的数据
    
    1.一级分类为"公司企业"归类于 工业功能-工业生产功能
        1.1 其中二级分类为"公司|公司企业|知名企业"归类于 "公司企业"
        1.2 其中二级分类为"工厂"归类于 "工厂"
        
    2.一级分类为"住宿服务"归类于 商业服务业功能-住宿服务功能
        2.1 其中二级分类为"住宿服务相关"且name含"酒店|宾馆"的归类于 "经济型酒店"
        2.2.1 其中二级分类为"宾馆酒店"且三级分类含"星级宾馆"的归类于 "星级酒店"
        2.2.2 其他二级分类为"宾馆酒店"的归类于 "经济型酒店"
        2.3 其中二级分类为"旅馆招待所"归类于 "招待所"
        2.4 其中name含"招待所"的归类于 "招待所"
        2.5 其余未分类的归类于"招待所"

    3.一级分类为"体育休闲服务"归类于 商业服务业功能-休闲娱乐功能
        3.1.1 其中二级分类为"体育休闲服务场所"且name含"健身|球|游泳|跆拳道"的归类于 "运动场所"
        3.1.2 其中二级分类为"体育休闲服务场所"且name含"影"的归类于 "影剧院"
        3.1.3 其余二级分类为"体育休闲服务场所"归类于 "娱乐场所"
        3.2 其中二级分类为"运动场馆"或"高尔夫相关"归类于 "运动场所"
        3.3 其中二级分类为"影剧院"归类于 "影剧院"
        3.4 其中二级分类为"娱乐场所"归类于 "娱乐场所"

    4.一级分类为"医疗保健服务"归类于 公共管理服务功能-医疗健康功能
        4.1 其中二级分类为"综合医院"或"急救中心"归类于 "医院"
        4.2.1 其中二级分类为"专科医院"且三级分类不含"诊所"的归类于 "医院"
        4.2.2 其中二级分类为"专科医院"且三级分类含"诊所"的归类于 "诊所"
        4.3 其中二级分类为"诊所"归类于 "诊所"
        4.4 其中二级分类为"疾病预防机构"归类于 "疾病预防中心"
        4.5.0 其中二级分类为"医疗保健服务场所"且name含"医院"的归类于 "医院"
        4.5.1 其中二级分类为"医疗保健服务场所"且name含"体检"的归类于 "专业体检机构"
        4.5.2 其中二级分类为"医疗保健服务场所"且name含"社区"的归类于 "社区服务站"
        4.5.3 其中二级分类为"医疗保健服务场所"且name含"预防"的归类于 "疾病预防中心"
        4.5.4 其中二级分类为"医疗保健服务场所"且name含"诊所"的归类于 "诊所"
        4.5.5 其中二级分类为"医疗保健服务场所"且name含"药房|药店"的归类于 "药店"
        4.5.6 其中二级分类为"医疗保健服务场所"且name含"宠物|动物"的归类于 "动物医院"   
        4.6 其中二级分类为"医药保健销售店"归类于 "药店"
        4.7 其中二级分类为"动物医疗场所"归类于 "动物医院"
        4.8 其余未分类的归类于"诊所"

    5.一级分类为"商务住宅"
        5.1 其中二级分类为"产业园区"归类于 工业功能-工业生产功能
        5.1.1 其中name含"工业"的归类于 "工业园区"
        5.1.2 其中name不含"工业"的归类于 "科技园区"
        5.2 其中二级分类为"住宅区"归类于 居住功能-居住生活功能-住宅小区
        5.3 其中二级分类为"楼宇"归类于 工业功能-工业生产功能-商务金融
        5.4.1 其中二级分类为"商务住宅相关"且name含"基地|软件园|产业园|科技园|电商园|物流园|创业园|创意园|智慧园|生态园|园区"的归类于 工业功能-工业生产功能-科技园区
        5.4.2 其中二级分类为"商务住宅相关"且name含"工业"的归类于 工业功能-工业生产功能-工业园区
        5.4.3 其中二级分类为"商务住宅相关"且name含"佳兆业|万科|寓|轩|府|居|阁|苑|庭|村|期|里|湾|小区|公馆|别墅|住宅|宿舍|社区|栋|单元"或name含"园"且未分类的归类于 居住功能-居住生活功能-住宅小区   
        5.4.4 其中二级分类为"商务住宅相关"且name含"商务|商业|大厦"的归类于 工业功能-工业生产功能-商务金融
        
    6.一级分类为"政府机构及社会团体"归类于 公共管理服务功能-行政管理功能
        6.1 其中二级分类为"政府机关"归类于 "政府机关"
        6.2.1 其中二级分类为"公检法机构"且三级分类含"公安警察|社会治安机构"的归类于 "公安机关"
        6.2.2 其中二级分类为"公检法机构"且三级分类为"检察院"的归类于 "检察院"
        6.2.3 其中二级分类为"公检法机构"且三级分类为"法院"的归类于 "法院"
        6.2.4.1 其中二级分类为"公检法机构"且三级分类为"公检法机关"且name含"公安|治安|派出所|警"的归类于 "公安机关"
        6.2.4.2 其中二级分类为"公检法机构"且三级分类为"公检法机关"且name含"检察"的归类于 "检察院"
        6.2.4.3 其中二级分类为"公检法机构"且三级分类为"公检法机关"且name含"司法|法院|法庭|仲裁"的归类于 "法院"
        6.3 其中二级分类为"工商税务机构"归类于 "工商税务机构"
        6.4.1 其中二级分类为"政府及社会团体相关"且name含"居委会|社区工作站|党群服务中心|街道办事处|委员会|办公室|局"的归类于 "政府机关"
        6.4.2 其中二级分类为"政府及社会团体相关"且name含"公安|治安|派出所|警"的归类于 "公安机关"
        6.4.3 其中二级分类为"政府及社会团体相关"且name含"检察"的归类于 "检察院"
        6.4.4 其中二级分类为"政府及社会团体相关"且name含"司法|法院|法庭|仲裁"的归类于 "法院"
        6.4.5 其中二级分类为"政府及社会团体相关"且name含"工商|税"的归类于 "工商税务机构"

    7.一级分类为"生活服务"归类于 商业服务业功能-生活服务功能
        7.1 其中二级分类为"邮局|物流速递"归类于 "物流服务"
        7.2 其中二级分类为"搬家公司|维修站点|洗衣店"归类于 "家政维修"
        7.3 其中二级分类为"彩票彩券销售点|旅行社|美容美发店|摄影冲印店|洗浴推拿场所|婴儿服务场所"归类于 "其他生活服务"
        7.4.1 其中二级分类为"生活服务场所"且name含"邮|菜鸟驿站|丰巢|自提|栈|柜|代理点|代办点|代收点"的归类于 "物流服务"
        7.4.2 其中二级分类为"生活服务场所"且name含"搬|修|洗衣|干洗"且不含"装修"的归类于 "家政维修"
        7.4.3 其中二级分类为"生活服务场所"且name含"彩票|旅|美容|美发|美甲|烫染|造型|沙龙|养生|理发|摄影|照相|相馆|冲印|影楼|图文|水会|浴|足|会所|按摩|推拿|桑拿|SPA|婴|亲子"归类于 "其他生活服务"
  
    8.一级分类为"科教文化服务"归类于 公共管理服务功能-文化教育功能
        8.1 其中二级分类为"传媒机构"归类于 "新闻媒体"
        8.2 其中二级分类为"博物馆"归类于 "博物馆"
        8.3 其中二级分类为"图书馆"归类于 "图书馆"
        8.4 其中二级分类为"学校"归类于 "学校"
        8.5 其中二级分类为"培训机构"归类于 "培训机构"
        8.6 其中二级分类为"科研机构"归类于 "科研机构"
        8.7.1 其中二级分类为"科教文化场所"且name含"传媒|传播|媒体|报社|电影|影视|影业|广播|电视|卫视|新闻|报业|记者|发行|杂志社|编辑"或以"报"结尾的归类于 "新闻媒体"
        8.7.2 其中二级分类为"科教文化场所"且name含"博物馆"的归类于 "博物馆"
        8.7.3 其中二级分类为"科教文化场所"且name含"图书馆"的归类于 "图书馆"
        8.7.4 其中二级分类为"科教文化场所"且name含"学校|学院|体校|分校|教育集团|党校|幼儿园|幼稚园|小学|中学|大学"或以"中|小|幼"结尾的归类于 "学校"
        8.7.5 其中二级分类为"科教文化场所"且name含"培|训|辅导|进修|考研|驾|琴|棋|书法|画|美术|舞蹈|作文|英语"或name含"教育"且未分类的的归类于 "培训机构"
        8.7.6 其中二级分类为"科教文化场所"且name含"实验|研究|设计院|研发"的归类于 "科研机构"

    9.一级分类为"购物服务"归类于 商业服务业功能-购物服务功能
        9.1 其中二级分类为"商场|特色商业街"归类于 "商场"
        9.2 其中二级分类为"超级市场"归类于 "超市"
        9.3 其中二级分类为"综合市场"归类于 "综合市场"
        9.4 其中二级分类为"家电电子卖场|家居建材市场|花鸟鱼虫市场"归类于 "专业市场"
        9.5 其中二级分类为"便民商店/便利店"归类于 "便利店"
        9.6.1 其中二级分类为"购物相关场所"且name含"商场|商城|购物|广场|街"且不含"("的归类于 "商场"
        9.6.2 其中二级分类为"购物相关场所"且name含"超市|屈臣氏|万宁|华润万|沃尔玛|山姆|家乐福|麦德龙|blt"的归类于 "超市"
        9.6.3 其中二级分类为"购物相关场所"且name含"果|菜|肉|禽|蛋|粮|油|水产|海鲜|批发|钱大妈"且未分类的归类于 "综合市场"
        9.6.4 其中二级分类为"购物相关场所"且name含"便利店|士多|美宜|全家|百里|天虹微喔|7-ELEVE|商店|商行|小卖部"的归类于 "便利店"
        9.6.5 其中二级分类为"购物相关场所"且name含"电|手机|通讯|通信|授权|音响|数码|空调|体验店|五金|照明|墙纸|瓷|寝|建材|装饰|卫浴|门窗|家具|玻璃|花|水族"的归类于 "专业市场"
        9.7 其中二级分类为"服装鞋帽皮具店|个人用品/化妆品店|体育用品店|文化用品店|专卖店"归类于 "专卖店"
        
    10.一级分类为"金融保险服务"
        10.1 其中二级分类含"银行"归类于 商业服务业功能-生活服务功能-金融服务
        10.2 其中二级分类为"证券公司|保险公司"归类于 工业功能-工业生产功能-商务金融
        10.4.1 其中二级分类为"金融保险服务机构"且name含"银行"的归类于 商业服务业功能-生活服务功能-金融服务
        10.4.2 其中二级分类为"金融保险服务机构"且name含"证券|险|人寿"的归类于 工业功能-工业生产功能-商务金融
        
    11.一级分类为"餐饮服务"归类于 商业服务业功能-餐饮服务功能
        11.1 其中二级分类为"中餐厅"归类于 "中餐厅"
        11.2 其中二级分类为"外国餐厅"归类于 "外国餐厅"
        11.3 其中二级分类为"快餐厅"归类于 "快餐店"
        11.4 其中二级分类为"咖啡厅|冷饮店|甜品店|糕饼店|茶艺馆"归类于 "饮料甜品店"
        
    12.一级分类为"风景名胜"且二级分类为"公园广场"归类于 公共管理服务功能-游憩功能-公园        
    '''
